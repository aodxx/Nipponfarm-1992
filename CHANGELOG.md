...
## 2026-09-20 (ต่อ) — แก้ระบบเข้าใช้งานไม่ได้

- ทำให้ Supabase client ตรวจสอบการมีอยู่ของ CDN/config ก่อนเริ่มต้น เพื่อไม่ให้หน้าเว็บล้มเมื่อ client สร้างไม่ได้
- เพิ่มการจัดการข้อผิดพลาดของ `getSession()` และ redirect ไปหน้า login พร้อมข้อความที่บอกสาเหตุเมื่อ session หายหรือเชื่อมต่อไม่ได้
- เพิ่มการตรวจสอบอีเมล/รหัสผ่านว่างก่อนเรียก Auth และแสดงข้อความข้อผิดพลาดภาษาไทยที่เข้าใจได้

## 2026-09-20 (ต่อ) — เฟส 2: แม่พันธุ์/ปฏิทิน

- แยกฟังก์ชันร่วม (`el`, `clearChildren`, `getDueMeta`, `showLoading`/`showError`/`showContent`, ฟังก์ชันวันที่) ออกจาก `js/app.js` ไปที่ `js/ui-helpers.js` ให้ทุกหน้าใช้ร่วมกัน — ลบ `js/mock-data.js` ทิ้ง (ไม่มีหน้าไหนใช้ mock data แล้ว)
- เพิ่ม component กลางใน `css/style.css`: `.card`, `.field`/`.field-row`, `.button-primary`/`.button-secondary`, `.data-table`, `.badge`, `.timeline`, `.page-nav`, `.agenda-date-heading` — `pages/login.html` ปรับให้ใช้ class กลางแทน inline `<style>` เดิม
- สร้าง `pages/pigs.html` + `js/pigs.js`: เพิ่ม/ดูคอก, เพิ่มแม่พันธุ์ใหม่, ดูรายชื่อหมูทั้งหมด, เลือกดู/บันทึกประวัติ (pig_events) ของแต่ละตัว
- สร้าง `pages/calendar.html` + `js/calendar.js`: agenda แบบช่วง 7 วัน (ก่อนหน้า/ถัดไป/วันนี้), เพิ่มงานใหม่ด้วยตนเอง, ทำเครื่องหมายงานเสร็จ
- เพิ่ม nav เชื่อมสามหน้า (ภาพรวม/แม่พันธุ์/ปฏิทิน) ในทุกหน้า
- ทดสอบ end-to-end จริงบน Supabase: เพิ่มโรงเรือน → คอก → แม่พันธุ์ → บันทึกเหตุการณ์ "ผสมพันธุ์" แล้วยืนยันว่า trigger อัปเดตสถานะเป็น "ท้อง" + คำนวณวันคลอดคาดการณ์ + สร้างงาน "ตรวจท้อง" อัตโนมัติถูกต้อง (ลบข้อมูลทดสอบออกหมดแล้ว)

## 2026-09-21

- แก้การคำนวณวันที่ใน `js/ui-helpers.js` ให้ใช้วันที่ตาม local timezone โดยตรง ป้องกันวันที่เลื่อนย้อนหลังในเขตเวลาไทย
- แก้ Overview ให้นับเฉพาะหมูที่ยังใช้งานอยู่ตอนคำนวณคอกที่มีหมู และปรับข้อความแหล่งข้อมูลให้ระบุว่าเป็นฐานข้อมูลจริง
- เพิ่ม migration `0004_fix_breeding_task_trigger.sql` ให้ trigger สร้างงานตรวจท้องได้แม้ยังไม่ได้รัน `seed.sql` โดยใช้ค่าเริ่มต้น 21 วันและอนุญาต `task_type_id` เป็น null
